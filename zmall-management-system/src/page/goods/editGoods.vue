<template>
  <div class="edit-goods">
    <div class="xmf-system-flex fzzj">编辑商品</div>
    <!-- 编辑商品表单 -->
    <el-card class="edit-goods-form" v-if="currentGoodsId">
...
export default {
  data () {
    return {
      ...
      // 商品数据表单验证规则
      rules: {
        goods_id: [{ required: true, message: '商品ID', trigger: 'blur' }],
        code: [{ required: true, message: '商品编码', trigger: 'blur' }],
        goods_name: [
  ...
  methods: {
        ...
        .then(res => {
          ...
          if (status === 200) {
            if (!data) {
              ...
              // 处理属性列表
              this.getAttribute(() => {
                this.handleAttribute(this.form.attribute)
              })

              // 处理商品参数数据
              this.handleParames(this.form.parameter)
            }
          } else {
    ...
    handleAttribute (attribute) {
      attribute.forEach(item => {
        ...
        val.forEach(item2 => {
          this.checkedAttributeValueList.push(
            `${item.name}-attributeName-${item2}`
          )
        })
      })
    },
  ...
  beforeRouteLeave (to, from, next) {
    // 导航离开该组件的对应路由时调用
    if (to.name !== 'editgoods' && !this.isSubmit) {
